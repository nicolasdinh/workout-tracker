...
type WeekGroup = { key: string; label: string; logs: WorkoutLog[] };
...
export default function HistoryPage({ refreshKey }: { refreshKey?: number }) {
  ...
  function getProgramName(id: string) {
    return programs.find((p) => p.id === id)?.name ?? 'Unknown Program';
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        {programs.length > 1 && (
          <select
            className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {filteredLogs.length === 0 && (
        <p className="text-center text-gray-400 dark:text-gray-500 py-8 text-sm">No logs for this program yet.</p>
      )}
